...
@pytest.mark.asyncio
async def test_set_manual_away_mode(api):
    """Set manual away mode"""
    lsystem: lennox_system = api.system_list[0]
    assert lsystem.sysId == "0000000-0000-0000-0000-000000000001"

    with patch.object(api, "publishMessageHelper") as mock_message_helper:
        await lsystem.set_manual_away_mode(True)
    ...
    with patch.object(api, "publishMessageHelper") as mock_message_helper:
        await lsystem.set_manual_away_mode(False)
        assert mock_message_helper.call_count == 1
        arg0 = mock_message_helper.await_args[0][0]
        assert arg0 == lsystem.sysId
        arg1 = mock_message_helper.await_args[0][1]
...
@pytest.mark.asyncio
async def test_set_smart_away_cancel(api):
...
async def test_enable_smart_away(api):
    ...
    assert lsystem.sysId == "0000000-0000-0000-0000-000000000001"
    with patch.object(api, "publishMessageHelper") as mock_message_helper:
        await lsystem.enable_smart_away(True)
        assert mock_message_helper.call_count == 1
        arg0 = mock_message_helper.await_args[0][0]
        assert arg0 == lsystem.sysId
        arg1 = mock_message_helper.await_args[0][1]
    ...
    with patch.object(api, "publishMessageHelper") as mock_message_helper:
        await lsystem.enable_smart_away(False)
        assert mock_message_helper.call_count == 1
        arg0 = mock_message_helper.await_args[0][0]
        assert arg0 == lsystem.sysId
        arg1 = mock_message_helper.await_args[0][1]
    ...
    with patch.object(api, "publishMessageHelper") as mock_message_helper:
        lsystem.sa_enabled = False
        with pytest.raises(S30Exception) as exc:
            await lsystem.enable_smart_away("Bogon")
        assert exc.value.error_code == EC_BAD_PARAMETERS
        assert mock_message_helper.call_count == 0
